...
```bash
mkdir panel && cd panel && sudo apt-get update -y && sudo apt-get install -y software-properties-common build-essential curl git unzip libssl-dev python3.11 python3.11-dev python3.11-distutils && nvm install 20 && nvm use 20 && export PYTHON=python3.11 && export npm_config_python=python3.11 && cd ~ && rm -rf AstroWax-Panel && git clone https://github.com/AstroVoidHostDev/AstroWax-Panel && cd AstroWax-Panel && unzip -o panel.zip && cd panel && rm -rf node_modules package-lock.json && npm cache clean --force && npm install --legacy-peer-deps && npm install connect-sqlite3 sqlite3@5.1.6 --build-from-source && npm run seed && npm run createUser && node .
