...
```bash
cd && sudo apt-get clean && sudo rm -rf /var/cache/apt/archives/* && sudo apt-get update -y && sudo dpkg --configure -a && sudo apt-get install -f -y && sudo apt-get install -y python3 python3-pip python3-dev build-essential libssl-dev make g++ curl ca-certificates gnupg apt-transport-https && sudo apt-get purge -y nodejs npm && sudo rm -rf /usr/local/bin/node /usr/local/bin/npm /usr/local/lib/node_modules && curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash - && sudo apt-get install -y nodejs git zip unzip --no-upgrade && git clone https://github.com/AstroVoidHostDev/AstroWax-Panel && cd AstroWax-Panel && unzip -o panel.zip && cd panel && rm -rf node_modules package-lock.json && npm cache clean --force && npm install --legacy-peer-deps && npm install connect-sqlite3 && npm rebuild sqlite3 --build-from-source && npm run seed && npm run createUser && node .
